...
class Service_A(Resource):          #Was hier noch fehlt, ist: 
                                    ...
                                    #Eine Überprüfung, ob der Artikel in der Menge auf Lager ist,  
    ...
    def post(self):
        ...
        neue_kd_nr=db.session.query(func.max(Kunde.kd_nr)).scalar()+1               #neue kd_nr
        neue_auft_nr=db.session.query(func.max(Auftrag.auft_nr)).scalar()+1         #neue auft_nr
        ...
        db.session.commit()
        print(f"Kunde -{neue_kd_nr}- angelegt")
        print(f"Auftrag -{neue_auft_nr}- angelegt")
        for best_position in bestellpositionen_data:
            print(f"Bestellposition angelegt")
        return {"message": "Alles erstellt."}, 201
